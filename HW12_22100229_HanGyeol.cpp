...
private:
    element heap[SIZE] ;
    int csize ; // 현재 원소의 개수
    void adjust(int t_root); // 주어진 root에서 heap구조에 알맞게 조정
public:
    my_heap();
    ...
    friend  int delete_node(element h[], int t_root, string tname, int n);
   // friend  void heap_adjuest(element h[], int t_root, int size);
};
...
/*
void heap_adjust(element h[], int t_root, int size){
    int parent, child;
    parent = t_root;
    child = parent*2;
}
*/
int delete_node(element h[],int t_root ,string tname , int n){
    element temp;
    
    if(t_root > n)
        return 0;
    if(tname == h[t_root].name){
        temp = h[t_root];
        h[t_root] = h[n];
       // heap_adjust(h , t_root , n);
        return 1;
    }
    if(delete_node(h, t_root*2, tname, n) == 1)
        return 1;
    
    return delete_node(h, t_root*2 +1, tname, n);
}

int my_heap::node_delete_by_name(string name){
    if(csize == 0)
        return 0;
    if(delete_node(heap,1,name,csize) == 1){
        csize --;
        return 1;
    }
    else
